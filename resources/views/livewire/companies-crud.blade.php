<div class="p-4 lg:p-8 max-w-[1400px] mx-auto w-full">
    ...
    <div class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            <div wire:click="closeModal" class="fixed inset-0 bg-black/60 backdrop-blur-sm transition-opacity" aria-hidden="true"></div>

            {{-- Modal Panel --}}
            <div class="inline-block align-bottom glass-effect rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full border border-glass-border">
                {{-- Header --}}
                <div class="px-6 py-4 border-b border-glass-border flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 bg-primary/20 rounded-lg flex items-center justify-center">
                            <span class="material-symbols-outlined text-primary">{{ $isEditMode ? 'edit' : 'add_business' }}</span>
                        </div>
                ...
                <form wire:submit.prevent="save">
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="flex items-center gap-4">
                                    <div class="w-20 h-20 rounded-lg overflow-hidden bg-white/5 border border-glass-border flex items-center justify-center">
                                        @if($logo)
                                        <img src="{{ $logo->temporaryUrl() }}" alt="Preview" class="w-full h-full object-cover">
                                        @elseif($logo_path)
                                        <img src="{{ Storage::url($logo_path) }}" alt="Logo actual" class="w-full h-full object-cover">
                                        @else
                                        <span class="material-symbols-outlined text-slate-400">business</span>
                                        @endif
                                    </div>
                                    <div class="flex-1">
                                        ...
                                        <label for="logo-upload" class="cursor-pointer inline-flex items-center gap-2 px-4 py-2 bg-white/5 border border-glass-border rounded-lg text-sm font-bold text-white hover:bg-white/10 transition-all">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="space-y-2">
                                    <label class="text-sm font-semibold text-slate-300">Nombre de la Compañía *</label>
                                ...
                                <div class="space-y-2">
                                    ...
                                    @error('ruc') <span class="text-red-400 text-xs">{{ $message }}</span> @enderror
                                </div>
                            </div>
                            ...
                            <div class="space-y-2">
                                ...
                                @error('slogan') <span class="text-red-400 text-xs">{{ $message }}</span> @enderror
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="space-y-2">
                                    ...
                                    @error('color_primary') <span class="text-red-400 text-xs">{{ $message }}</span> @enderror
                                </div>
                                ...
                                <div class="space-y-2">
                                    ...
                                    @error('plan') <span class="text-red-400 text-xs">{{ $message }}</span> @enderror
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div class="space-y-2">
                                    ...
                                    @error('facebook') <span class="text-red-400 text-xs">{{ $message }}</span> @enderror
                                </div>
                            </div>
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="text" wire:model="instagram"
                                    class="w-full bg-white/5 border border-glass-border rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all placeholder:text-slate-500"
                                    placeholder="@empresa">
                                @error('instagram') <span class="text-red-400 text-xs">{{ $message }}</span> @enderror
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="px-6 py-4 border-t border-glass-border flex items-center justify-end gap-3">
                        ...
                        <button type="submit"
                            ...
                            <span class="material-symbols-outlined text-lg">{{ $isEditMode ? 'save' : 'add' }}</span>
